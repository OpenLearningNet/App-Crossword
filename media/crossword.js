...
var CrosswordUtils = {
    ...
    toHtml : function(grid, show_answers, imageDelegate){
        if (!imageDelegate) {
            imageDelegate = function(label) {
                return CrosswordUtils.PATH_TO_PNGS_OF_NUMBERS + label + ".png";
            }
        }
        ...
        for(var r = 0; r < grid.length; r++){
            ...
            for(var c = 0; c < grid[r].length; c++){
                ...
                if (is_start_of_word) {
                    var img_url = imageDelegate(label);
                    html.push("<td id='crossword-" + r + "-" + c + "'" + extra_data + " data-row='" + r + "' data-col='" + c + "' class='crossword-cell start " + css_class + "' style=\"background-image:url('" + img_url + "')\">");
                    label++;			
                } else {
